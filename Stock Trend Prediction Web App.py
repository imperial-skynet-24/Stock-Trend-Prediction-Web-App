...
import yfinance as yf, pandas as pd, numpy as np, matplotlib as mpl, matplotlib.pyplot as plt
...
with st.container() :
	...
	with st.container() :
		...
		def date_input() :
			...
			end_date = end_date_column.date_input(label = '**Enter end date :**',
                                         		value = date.today() - relativedelta(days = 1), min_value = start_date + timedelta(weeks = 1),
	...
	data = whole_data[(pd.to_datetime(whole_data.Date) >= pd.to_datetime(START)) & (pd.to_datetime(whole_data.Date) <= pd.to_datetime(END))]
...
with st.container() :
	...
	forecast_params.text('')
	forecast_params.text('')
	include_history = forecast_params.radio("Include historical data :", [True, False], horizontal = True, help = f'Inclusion of historical data lying between {START} and {END}')
	forecast_params.text('')
	...
	def prophet_modelling(data, data_attribute, daily_seasonality, weekly_seasonality, yearly_seasonality, period, include_history) :
		...
		model = Prophet(daily_seasonality = daily_seasonality, weekly_seasonality = weekly_seasonality, yearly_seasonality = yearly_seasonality, changepoint_range = 1, changepoint_prior_scale = 0.75)
	...
	forecast_dates_data.expander(label = 'Expand to view tabular forecasted data :').dataframe(forecast[['Date', 'yhat_lower', 'yhat_upper', 'yhat', 'trend_lower', 'trend_upper', 'trend']].set_index('Date'), use_container_width = True)
	...
	def model_forecast(data_model, data_forecast) :
		forecast_chart.markdown(f"<h6 style = 'text-align: left;'> Forecast plot for {selected_stock} from {END + timedelta(days = 1)} to {END + timedelta(days = forecast_days) + timedelta(weeks = forecast_weeks) + relativedelta(months = forecast_months)  + relativedelta(years = forecast_years)} :</h6>", unsafe_allow_html = True)
...
with st.expander(label = 'Expand to view') :
	...
	def date_input():
		train_start_date = start_date_column.date_input(label = '**Enter start date for training data** :',
														...
														min_value = ipo_launch, max_value = datetime.today() - timedelta(days = 2),
														key = 'train_start_date', help = 'Enter the start date to train the model from')

		train_end_date = end_date_column.date_input(label = '**Enter end date for training data** :', 
													value = train_start_date + timedelta(weeks = 10),
		...
		test_start_date = start_date_column.date_input(label = '**Enter start date for testing data** :',
														value = train_end_date + timedelta(days = 1),
		...
		test_end_date = end_date_column.date_input(label = '**Enter end date for testing data** :', value = (test_start_date + timedelta(days = 1)),
													min_value = train_start_date + timedelta(days = 10), max_value = datetime.today() - timedelta(days = 1), key = 'test_end_date',
													help = 'End date to view the stock to *(can be a day later than start date)*')
		...
		return train_start_date, train_end_date, test_start_date, test_end_date, performance_attribute, performance_daily_seasonality, performance_weekly_seasonality, performance_yearly_seasonality, performance_train_data, performance_test_data

	train_start_date, train_end_date, test_start_date, test_end_date, performance_attribute, performance_daily_seasonality, performance_weekly_seasonality, performance_yearly_seasonality, performance_train_data, performance_test_data = date_input()
	...
	actual_plot = go.Scatter(x = performance_test_data['Date'], y = performance_test_data[performance_attribute], mode = 'markers', name = 'Actual Data', marker = {'color' : 'blue'})
	...
	mae_, mse_, rmse_, mape_, smape_ = st.columns([3, 3, 3, 4, 4], gap = 'small')
	...
	mape_.metric(label = '**Mean Absolute Percentage Error**', value = f'{round(mape, 2)} %')
	smape_.metric(label = '**Symmetric Mean Absolute Percentage Error**', value = f'{round(smape, 2)} %')
...
with st.expander(label = 'Expand to view') :
	...
	st.text('')
	with st.container() :
		def data_description(data, column, start_date, end_date) :
			column.dataframe(data = data.describe(), use_container_width = True)
			column.download_button(label = 'Download Data Description', data = data.describe().to_csv().encode('utf-8'), file_name = (f'Description - {selected_stock} - {start_date} to {end_date}.csv'))


		stock_data, forecast_data = st.columns(2, gap = 'large')
		stock_data.markdown("<h3 style='text-align: center;'> Historical Data </h3>", unsafe_allow_html=True)
		forecast_data.markdown("<h3 style='text-align: center;'> Forecast Data </h3>", unsafe_allow_html=True)
		data_description(data, stock_data, START, END)
		data_description(forecast[['yhat', 'trend', 'yhat_lower', 'yhat_upper', 'trend_lower', 'trend_upper']], forecast_data, forecast.ds.min(), forecast.ds.max())
	...
	st.markdown("<h2 style='text-align: center;'> Stationarity Test </h2>", unsafe_allow_html=True)
	st.text('')
	with st.container() :
		def stationarity_test(data, lag, regression, column) :
			kpssresult = kpss(data, nlags = lag, regression = regression)

			if (kpssresult[1] > 0.05) : column.markdown(f"""The data is **:green[STATIONARY]**

				1) Test Statistic : {kpssresult[0]}
	2) p-value : {kpssresult[1]}
	3) Critical Values : {kpssresult[3]}""")

			elif (kpssresult[1] < 0.05) : column.markdown(f"""The data is **:red[NON-STATIONARY]**
	...
	3) Critical Values : {kpssresult[3]}""")
		...
		data_lag = lag_input.number_input(label = 'Enter number of lags', min_value = 1, value = 1, step = 1, format = '%d', key = 'stock_data_lag')
		data_regression = regression_input.selectbox(label = 'Enter regression type :', options = ('c', 'ct'), key = 'stock_data_regression')
		stationarity_test(data[stationarity_data_attribute], data_lag, data_regression, stock_data)

		stationarity_forecast_attribute_input, forecast_lag_input, forecast_regression_input = forecast_data.columns(3, gap = 'small')
		stationarity_forecast_attribute = stationarity_forecast_attribute_input.selectbox(label = 'Select data attribute :', options = forecast[['yhat', 'trend', 'yhat_lower', 'yhat_upper', 'trend_lower', 'trend_upper']].columns, key = 'forecast_stationarity')
		forecast_lag = forecast_lag_input.number_input(label = 'Enter number of lags', min_value = 1, value = 1, step = 1, format = '%d', key = 'forecast_lag')
		forecast_regression = forecast_regression_input.selectbox(label = 'Enter regression type :', options = ('c', 'ct'), key = 'forecast_regression')
		stationarity_test(forecast[stationarity_forecast_attribute], forecast_lag, forecast_regression, forecast_data)
	...
	st.markdown("<h2 style='text-align: center;'> Auto-Correlation and Partial Auto-Correlation </h2>", unsafe_allow_html=True)
	st.text('')
	with st.container() :
		def auto_correlation(data, pacf_method, lags, column, start_date, end_date) :
			mpl.rc("figure", figsize = (15, 4))

			acf_values = acf(data, nlags = lags+1)
			pacf_values = pacf(data, nlags = lags+1, method = pacf_method)

			column.write("Auto-correlation and partial auto-correlation values :")
			auto_corr_df = pd.DataFrame({'Lag' : list(range(0, lags+1)), 'ACF' : acf_values[:-1], 'PACF' : pacf_values[:-1]}).set_index('Lag')
			column.dataframe(data = auto_corr_df, use_container_width = True)
			column.download_button(label = 'Download ACF and PACF Data', data = auto_corr_df.to_csv().encode('utf-8'), file_name = (f'ACF and PACF Values - {selected_stock} - {start_date} to {end_date}.csv'))
		...
		pacf_method = pacf_method_input.selectbox(label = 'Partial auto-correlation method :', options = ('ywadjusted', 'ywmle', 'ols', 'ols-inefficient', 'ols-adjusted', 'ldadjusted', 'ldbiased'), key = 'stock_pacf_autocorrelation')

		auto_correlation(data[data_attribute], pacf_method, lags, stock_data, START, END)
		...
		data_attribute = data_attribute_input.selectbox(label = 'Select data attribute :', options = forecast[['yhat', 'trend', 'yhat_lower', 'yhat_upper', 'trend_lower', 'trend_upper']].columns, key = 'forecast_attribute_autocorrelaton')
		...
		auto_correlation(forecast[data_attribute], pacf_method, lags, forecast_data, forecast.ds.min(), forecast.ds.max())
	...
	st.markdown("<h2 style='text-align: center;'> Seasonal Decomposition </h2>", unsafe_allow_html=True)
	st.text('')
	with st.container() :
		def seasonal_decomposition(date_column, data, model, period, extrapolate_trend, column, start_date, end_date) :
			result = seasonal_decompose(x = data, model = model, period = period, extrapolate_trend = extrapolate_trend)
   
			decomposition_table = pd.DataFrame({'Date' : date_column, 'Observed' : result.observed, 'Trend' : result.trend, 'Seasonality' : result.seasonal, 'Residual Noise' : result.resid}).set_index('Date')
			column.dataframe(data = decomposition_table, use_container_width = True)
			column.download_button(label = 'Download decomposition data', data = decomposition_table.to_csv().encode('utf-8'), file_name = (f'Forecast ACF Values - {selected_stock} - {start_date} to {end_date}.csv'))

			decomposition_figure = subplots.make_subplots(rows = 4, cols = 1, subplot_titles = ['Observed', 'Trend', 'Seasonal', 'Residual Noise'])
			decomposition_figure.add_trace(go.Scatter(x = date_column, y = result.observed, mode = 'lines', name = 'Observed'), row = 1, col = 1)
			decomposition_figure.add_trace(go.Scatter(x = date_column, y = result.trend, mode = 'lines', name = 'Trend'), row = 2, col = 1)
			decomposition_figure.add_trace(go.Scatter(x = date_column, y = result.seasonal, mode = 'lines', name = 'Seasonal'), row = 3, col = 1)
			decomposition_figure.add_trace(go.Scatter(x = date_column, y = result.resid, mode = 'lines', name = 'Residual'), row = 4, col = 1)
			decomposition_figure.update_layout(height = 900, title = f"<b>Seasonal Decomposition Plot</b>", margin = {'t':100}, title_x = 0.5, showlegend = True)

			column.text('')

			column.plotly_chart(decomposition_figure)
		...
		seasonal_decomposition(data['Date'], data[attribute], model, period, extrapolate_trend, stock_data, START, END)


		attribute_input, model_input, period_input, extrapolate_trend_input = forecast_data.columns(4, gap = 'small')

		attribute = attribute_input.selectbox(label = 'Select data attribute :', options = forecast[['yhat', 'trend', 'yhat_lower', 'yhat_upper', 'trend_lower', 'trend_upper']].columns, key = 'forecast_attribute_decomposition')
		...
		seasonal_decomposition(forecast['Date'], forecast[attribute], model, period, extrapolate_trend, forecast_data, forecast.ds.min(), forecast.ds.max())