...
with st.container() :
	...
	forecast_dates_data.expander(label = 'Expand to view tabular forecasted data :').dataframe(forecast[['ds', 'yhat_lower', 'yhat_upper', 'yhat', 'trend_lower', 'trend_upper', 'trend']].set_index('ds'), use_container_width = True)
...
with st.expander(label = 'Expand to view') :
	...
	with st.container() :
		...
		stock_data.markdown("<h3 style='text-align: center;'> Stock Data </h3>", unsafe_allow_html=True)
		...
		data_description(forecast, forecast_data, forecast.ds.min(), forecast.ds.max())
	...
	with st.container() :
		def seasonal_decomposition(date_column, data, model, period, extrapolate_trend, column, start_date, end_date) :
			...
			decomposition_table = pd.DataFrame({'Observed' : result.observed, 'Trend' : result.trend, 'Seasonality' : result.seasonal, 'Residual Noise' : result.resid})
		...
		seasonal_decomposition(forecast['ds'], forecast[attribute], model, period, extrapolate_trend, forecast_data, forecast.ds.min(), forecast.ds.max())